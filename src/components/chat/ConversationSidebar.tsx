      ...
      <div className="flex-1 overflow-y-auto px-2 pb-4">
        {pinned.length > 0 && (
          <>
            ...
            {pinned.map(c => <ConvoItem key={c.id} convo={c} active={activeId === c.id} onSelect={onSelect} />)}
        ...
        {recents.length > 0 && (
          <>
            ...
            {recents.map(c => <ConvoItem key={c.id} convo={c} active={activeId === c.id} onSelect={onSelect} />)}
          </>
        )}

        {filtered.length === 0 && (
          <p className="text-center text-body-xs text-ink-tertiary py-8">No conversations found.</p>
        )}
      </div>
    </aside>
  )
...
function ConvoItem({ convo, active, onSelect }: { convo: Conversation; active: boolean; onSelect: (id: string) => void }) {
  return (
    ...
    >
      <div className={cn('w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5', active ? 'bg-sage-200 text-sage-700' : 'bg-raised text-ink-tertiary')}>
        <MessageCircle size={12} strokeWidth={2} />
      </div>
      <div className="flex-1 min-w-0">
        <p className={cn('text-body-xs font-medium truncate', active ? 'text-sage-800' : 'text-ink')}>{convo.title}</p>
        <p className="text-body-xs text-ink-tertiary truncate mt-0.5">{truncate(convo.preview, 45)}</p>
