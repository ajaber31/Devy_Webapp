const personas = [
  {
    ...
    description:
      'Get clear, supportive guidance on navigating your child\'s needs at home. From sensory strategies to communication tools — ask anything, at any time.',
    color: 'sage' as const,
    useCases: ['Behavior & regulation', 'Sensory support', 'Sleep & routines', 'School advocacy'],
    icon: (
  ...
  },
  {
    ...
    description:
      'Find classroom strategies, IEP goal templates, and accommodation guidance rooted in current best practice. Support every student more effectively.',
    color: 'dblue' as const,
    useCases: ['Classroom accommodations', 'IEP goal writing', 'Behavior support plans', 'Transition planning'],
    icon: (
      <svg className="w-7 h-7" viewBox="0 0 28 28" fill="none">
  ...
  },
  {
    ...
    description:
      'Access clinical references, intervention frameworks, and literature quickly. Devy helps you spend less time searching and more time with your clients.',
    color: 'sand' as const,
    useCases: ['Clinical guidelines', 'Intervention strategies', 'Family education', 'Progress documentation'],
    icon: (
      <svg className="w-7 h-7" viewBox="0 0 28 28" fill="none">
  ...
  },
  {
    ...
    description:
      'From psychologists to social workers and case managers — get fast, reliable answers to complex support questions without endless document searching.',
...
const colorMap = {
  sage:    { bg: 'bg-sage-100',   icon: 'text-sage-600',   accent: 'bg-sage-500' },
  dblue:   { bg: 'bg-dblue-100',  icon: 'text-dblue-600',  accent: 'bg-dblue-500' },
  sand:    { bg: 'bg-sand-100',   icon: 'text-sand-500',   accent: 'bg-sand-500' },
  neutral: { bg: 'bg-raised',     icon: 'text-ink-secondary', accent: 'bg-ink' },
}
  ...
  return (
    <section id="who-its-for" className="py-24 bg-surface relative overflow-hidden">
      <div
        ...
        style={{
          backgroundImage: 'radial-gradient(ellipse 60% 50% at 10% 50%, rgba(92,134,81,0.07) 0%, transparent 55%), radial-gradient(ellipse 50% 60% at 90% 50%, rgba(79,115,159,0.05) 0%, transparent 50%)',
        }}
      />
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-body-lg text-ink-secondary max-w-xl mx-auto">
            Whether you&apos;re at home, in the classroom, or in the clinic — Devy meets you where you are.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {personas.map((persona) => {
            ...
            return (
              <div
                key={persona.title}
                className="bg-white rounded-card-lg p-6 shadow-card hover:shadow-card-hover border border-border/50 flex flex-col group"
                style={{
              ...
              >
                <div className={`w-12 h-12 rounded-card ${colors.bg} ${colors.icon} flex items-center justify-center mb-4 group-hover:scale-105 transition-transform duration-200 ease-spring`}>
                  {persona.icon}
                </div>
