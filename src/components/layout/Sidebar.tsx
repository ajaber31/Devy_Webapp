          ...
          {!collapsed && (
            <>
              <div className="flex-1 min-w-0 text-left">
                ...
                <p className="text-body-xs text-ink-tertiary truncate capitalize">{mockCurrentUser.role}</p>
              </div>
              <ChevronDown
                size={14}
                className={cn('text-ink-tertiary flex-shrink-0 transition-transform duration-150', userMenuOpen ? 'rotate-180' : '')}
              />
            </>
          )}
        </button>
        ...
        {userMenuOpen && !collapsed && (
          <div className="absolute bottom-full left-2 right-2 mb-1 bg-white rounded-card shadow-floating border border-border py-1 z-20">
            <div className="px-3 py-2 border-b border-border mb-1">
              <p className="text-body-xs font-medium text-ink">{mockCurrentUser.name}</p>
              <p className="text-body-xs text-ink-tertiary">{mockCurrentUser.email}</p>
            </div>
            <Link href="/settings" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-2 px-3 py-2 text-body-sm text-ink-secondary hover:bg-raised hover:text-ink transition-colors duration-150">
              Account settings
            </Link>
            <Link href="/login" className="flex items-center gap-2 px-3 py-2 text-body-sm text-danger hover:bg-raised transition-colors duration-150">
              <LogOut size={14} />
              Sign out
            </Link>
          </div>
