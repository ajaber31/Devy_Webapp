...
export function PrivacyDataSection({ profile, auditLog }: PrivacyDataSectionProps) {
  ...
  const tp = t.privacyDetails
  const eventLabels = tp.eventLabels as Record<string, string>
  ...
  const consentDate = profile.consentAcceptedAt
    ? new Date(profile.consentAcceptedAt).toLocaleDateString(dateLocale, {
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl border border-border shadow-card overflow-hidden">
        <div className="px-6 py-4 border-b border-border flex items-center gap-2.5">
          ...
          <h3 className="font-display text-[1.05rem] font-semibold text-ink">{tp.consentTitle}</h3>
        </div>
        <div className="px-6 py-5 space-y-4">
          <div className={`flex items-start gap-3 p-4 rounded-xl border ${consentOk ? 'bg-sage-50 border-sage-200' : 'bg-sand-50 border-sand-200'}`}>
            ...
            <div>
              <p className={`text-body-sm font-semibold ${consentOk ? 'text-sage-700' : 'text-sand-700'}`}>
                {consentOk ? tp.currentPolicyAccepted : tp.consentRequired}
              </p>
              <p className="text-body-xs text-ink-tertiary mt-0.5">
                {consentOk
                  ? tp.acceptedVersion.replace('{version}', String(profile.consentVersion)).replace('{date}', consentDate ?? '')
                  : tp.notAccepted}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-body-xs text-ink-tertiary">
            <span>{tp.currentPolicyVersion} <span className="font-mono text-ink-secondary">{CURRENT_CONSENT_VERSION}</span></span>
            <a
              href="/privacy"
              target="_blank"
            ...
            >
              {tp.viewPolicy} <ExternalLink size={11} strokeWidth={2} />
            </a>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-border shadow-card overflow-hidden">
        <div className="px-6 py-4 border-b border-border">
          <h3 className="font-display text-[1.05rem] font-semibold text-ink">{tp.yourDataRights}</h3>
          <p className="text-body-xs text-ink-tertiary mt-0.5">{tp.dataRightsDesc}</p>
        </div>
        <div className="divide-y divide-border">
          ...
          <div className="px-6 py-5 flex items-start justify-between gap-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-body-sm font-semibold text-ink">{tp.exportTitle}</p>
            ...
            >
              ...
              {isPendingExport ? tp.preparing : tp.exportButton}
            </button>
          </div>
          ...
          <div className="px-6 py-5 flex items-start justify-between gap-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-body-sm font-semibold text-ink">{tp.deleteTitle}</p>
                <p className="text-body-xs text-ink-tertiary mt-0.5 max-w-sm">
                  {tp.deleteDesc}
                </p>
              </div>
      ...
      <div className="bg-white rounded-2xl border border-border shadow-card overflow-hidden">
        <div className="px-6 py-4 border-b border-border flex items-center justify-between">
          <div>
            <h3 className="font-display text-[1.05rem] font-semibold text-ink">{tp.activityLogTitle}</h3>
            <p className="text-body-xs text-ink-tertiary mt-0.5">{tp.activityLogDesc}</p>
          </div>
          <Clock size={15} className="text-ink-tertiary" strokeWidth={1.75} />
        </div>
        <div className="divide-y divide-border/60">
          {auditLog.length === 0 ? (
            <div className="px-6 py-8 text-center text-body-xs text-ink-tertiary">{tp.noEvents}</div>
          ) : (
            auditLog.map((entry) => (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {entry.event_data?.consent_version != null && (
                    <p className="text-[0.65rem] text-ink-tertiary">
                      {tp.policyVersionLabel} <span className="font-mono">{String(entry.event_data.consent_version)}</span>
                    </p>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-floating border border-border w-full max-w-md p-6 space-y-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-display font-bold text-ink">{tp.deleteModalTitle}</h3>
                <p className="text-body-xs text-ink-tertiary mt-0.5">{tp.deleteModalSubtitle}</p>
              </div>
            </div>

            <div className="bg-red-50 border border-red-200 rounded-xl p-4 text-body-xs text-red-700 space-y-1">
              <p className="font-semibold">{tp.willDelete}</p>
              <ul className="list-disc list-outside pl-4 space-y-0.5">
                <li>{tp.bulletProfile}</li>
                <li>{tp.bulletChildren}</li>
                <li>{tp.bulletConversations}</li>
              </ul>
              <p className="mt-2 font-medium">{tp.auditRetention}</p>
            </div>

            <div className="space-y-2">
              <label className="text-body-xs font-medium text-ink">
                {tp.typeDelete} <span className="font-mono font-bold">DELETE</span> {tp.toConfirm}
              </label>
              <input
                type="text"
                value={deleteConfirm}
            ...
            <div className="flex gap-3">
              ...
              >
                ...
                {isPendingDelete ? tp.deleting : tp.deleteMyAccount}
              </button>
            </div>
          </div>
        </div>
